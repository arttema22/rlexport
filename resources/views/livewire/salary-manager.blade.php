<div class="m-4 p-4 overflow-hidden border sm:rounded-lg">
    <div class="font-sans overflow-x-auto">
        <div class="flex justify-between p-2">
            <h2 class="text-xl font-bold border-b-2 inline-block border-[#2A4523] pb-1 capitalize">{{__('Salaries')}}
            </h2>
            <button wire:click="create" type="button"
                class="w-10 h-10 inline-flex items-center justify-center rounded-full outline-none bg-red-500 hover:bg-red-700">
        ...
        <div class="font-[sans-serif] space-y-2 max-w-6xl mx-auto">
            @foreach ( $salaries as $salary )
            <div x-data="{ open: false }" class="bg-white shadow-[0_2px_4px_0px_rgba(0,0,0,0.15)] p-2 rounded-md"
                role="accordion">
                ...
                <div x-show="open" class="flex justify-between items-center p-1 bg-gray-100 rounded-md">
                    <div class="text-xs text-gray-600 leading-relaxed">
                        {{__('Created At')}} {{$salary->created_at}}
                        {{__('Updated At')}} {{$salary->updated_at}}
                        {{__('Owner')}} {{$salary->owner->name}}
                        {{__('Driver')}} {{$salary->driver->name}}
                    </div>
                    <div class="flex">
                        <button wire:click="edit({{ $salary->id }})" class="mr-2" title="Edit">
                        ...
                        <button wire:click="confirmDelete({{ $salary->id }})" title="Delete">
